...
from ..utils import JSONableMixin, hydra_dataclass
...
class MetricCategories(enum.StrEnum):
...
class TimeToEventGenerationHeadType(enum.StrEnum):
...
class AttentionLayerType(enum.StrEnum):
...
DOC_STR = """The configuration class for Event Stream GPT models.
...
class StructuredTransformerConfig(PretrainedConfig):
    def __init__(
        ...
        **kwargs,
        ...
        missing_param_err_tmpl = "For a CI model, {{}} should not be None"
        extra_param_err_tmpl = "For a CI model, {{}} is not used; got {{}}. Setting " "to None."
        if do_full_block_in_seq_attention is not None:
            logger.warning(
                extra_param_err_tmpl.format("do_full_block_in_seq_attention", do_full_block_in_seq_attention)
            )
