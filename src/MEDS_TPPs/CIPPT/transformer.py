...
def time_from_deltas(batch: dict) -> torch.Tensor:
...
class LearnableFrequencySinusoidalTemporalPositionEncoding(torch.nn.Module):
    """A module for applying time-based position encodings to a dict.
...
class ConditionallyIndependentPointProcessInputLayer(torch.nn.Module):
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        ...
        Args:
            batch: A dict instance containing input data.
        """
